    ...
    * 列表
            ...
            关于列表的问题:
                ...
                列表中元素是如何存储的?
                    ...
                    每个item都会开辟一块内存空间, 列表的index指向这块内存空间
                    注意 如果是基本类型, 且列表中的item有相同的数据,则这些相同的数据是多个index指向同一块内存空间

                列表提供了哪些基本的操作?

                    查询(in )/新增(append, insert)/删除/修改
    ...
    * 队列
            ...
            队列的实现原理
                ...
                这种实现的问题?
                    ...
                    1. 数组固定长度 出队后数组长度实际不变空位置还占用内存
                       (其他语言数组也不会自增自减长度)

                改进方案:
                    环形队列
                        将列表首尾逻辑上连接起来
                        只要元素不超过队列的长度 内存实际上是可以复用的

                    如何实现环形? 对数组的位置对maxlength求余
